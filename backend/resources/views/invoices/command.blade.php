...
<body>
    <div class="header">
        <img class="logo" src="https://giftoria.me/images/logo/logoFooter.svg" alt="Giftoria" />
        <h1>Invoice #{{ $command->id }}</h1>
    ...
    <div class="section">
        <h2>Order Summary</h2>
        <table>
            <tr><th>Order Date</th><td>{{ \Carbon\Carbon::parse($command->placed_at)->format('F d, Y H:i') }}</td></tr>
            <tr><th>Status</th><td>{{ ucfirst($command->status) }}</td></tr>
            <tr><th>Customer</th><td>{{ ($command->customer_first_name.' '.$command->customer_last_name) ?: 'N/A' }}</td></tr>
            <tr><th>Email</th><td>{{ $command->customer_email ?? 'N/A' }}</td></tr>
        </table>
    </div>
    ...
    <div class="section">
        <h2>Gift Card</h2>
        <table>
            <tr><th>Template</th><td>{{ $command->giftCardTemplate->title ?? 'Template' }}</td></tr>
            <tr><th>Message</th><td>{{ $command->gift_card_message ?? '—' }}</td></tr>
            <tr><th>Signature Type</th><td>{{ $command->giftCardTemplate->signature_type ?? '—' }}</td></tr>
        </table>
    </div>
    @endif
